...
UNET_PATH = "../checkpoints/ootd/ootd_hd/checkpoint-36000"  # Optional: switch to ootd_dc if needed
...
class NepWear:
    def __init__(self, gpu_id=0):
        ...
        unet_garm = UNetGarm2DConditionModel.from_pretrained(
            UNET_PATH, subfolder="unet_garm", torch_dtype=torch.float16, use_safetensors=True
        )
    ...
    def _tokenize_caption(self, caption: str, max_length=3):
        tokens = self.tokenizer(caption, max_length=max_length, padding="max_length", truncation=True, return_tensors="pt")
    ...
    def __call__(self,
        ...
        with torch.no_grad():
            ...
            garm_embed = self.image_encoder(garm_input["pixel_values"]).image_embeds.unsqueeze(1)

            # Encode "dress" category text
            text_embed = self.text_encoder(self._tokenize_caption(["dress"]))[0]

            # Combine text and image embeddings
            prompt_embeds = torch.cat([text_embed, garm_embed], dim=1)
