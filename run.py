...
# from inference.inference_Nephd import NepWearHD
...
# if model_type == "hd":
#     model = NepWearHD(args.gpu_id)
# elif model_type == "dc":
#     model = NepWear(args.gpu_id)
# else:
...
import argparse
parser = argparse.ArgumentParser(description='run NepWear')
parser.add_argument('--gpu_id', '-g', type=int, default=0, required=False)
parser.add_argument('--model_path', type=str, default="", required=True)
parser.add_argument('--cloth_path', type=str, default="", required=True)
...
if model_type == "hd":
    raise ValueError("HD model currently supports only upperbody. Use 'dc' model type for dress category.")

model = NepWear(args.gpu_id)  # Only NepWear (dc) model for dress
...
if __name__ == '__main__':
    ...
    images = model(
        model_type=model_type,
        category=category_dict[category],  # always 'dress'
        image_garm=cloth_img,
        image_vton=masked_vton_img,
        mask=mask,
        image_ori=model_img,
    ...
    for idx, image in enumerate(images):
        image.save(f'./images_output/out_{model_type}_{idx}.png')
